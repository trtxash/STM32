...
ControlVelocity代表增量输出
**************************************************************************/
int Angle_FeedbackControl(double Target, double Current)
{
    double Rate;                                                                            // 定义相关变量
    static double ControlVelocity;                                                          // 定义控制输出
    static double PID_OUT = 0;                                                              // PID输出
    ...
    static double Current_Error = 0, Last_Error = 0;                                        //当前误差  最后误差
    static double Sum_Error = 0;                                                            //误差积分
    static double PID_I_MAX = 10.0, PID_I_MIN = -10.0, V_DATA_MAX = 100, V_DATA_MIN = -100; // PID积分上限，PID积分下限

    Current_Error = Target - Current; //求速度偏差
    P_OUT = P_A * Current_Error;      //比列项

    Sum_Error += Current_Error; //误差积分
    I_OUT = I_A * Sum_Error;    //积分项
    if (I_OUT > PID_I_MAX)      //积分限幅处理,不能超过最大值不能低于最小值
    {
        I_OUT = PID_I_MAX;
    }
