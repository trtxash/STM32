...
{
	__HAL_TIM_CLEAR_FLAG(&USMARTTimer_HandleTypeDef, TIM_FLAG_UPDATE); // 清除中断标志位
	__HAL_TIM_SET_AUTORELOAD(&USMARTTimer_HandleTypeDef, 0XFFFF);	   // 将重装载值设置到最大
...
{
	if (__HAL_TIM_GET_FLAG(&USMARTTimer_HandleTypeDef, TIM_FLAG_UPDATE) == SET) // 在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime += 0XFFFF;
	}
	usmart_dev.runtime += __HAL_TIM_GET_COUNTER(&USMARTTimer_HandleTypeDef);
...
// {
// 	if (__HAL_TIM_GET_IT_SOURCE(&USMARTTimer_HandleTypeDef, TIM_IT_UPDATE) == SET) // 溢出中断
// 	{
// 		usmart_dev.scan(); // 执行usmart扫描
// 		__HAL_TIM_SET_COUNTER(&USMARTTimer_HandleTypeDef, 0);
// 		;											  // 清空定时器的CNT
// 		__HAL_TIM_SET_AUTORELOAD(&USMARTTimer_HandleTypeDef, 1000 - 1); // 恢复原来的设置
// 	}
// 	__HAL_TIM_CLEAR_IT(&USMARTTimer_HandleTypeDef, TIM_IT_UPDATE); // 清除中断标志位
// }
...
// 	USMARTTimer_HandleTypeDef.Instance = TIM4;						// 通用定时器4
// 	USMARTTimer_HandleTypeDef.Init.Prescaler = psc;					// 分频
// 	USMARTTimer_HandleTypeDef.Init.CounterMode = TIM_COUNTERMODE_UP; // 向上计数器
// 	USMARTTimer_HandleTypeDef.Init.Period = arr;						// 自动装载值
// 	USMARTTimer_HandleTypeDef.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
// 	HAL_TIM_Base_Init(&USMARTTimer_HandleTypeDef);
// 	HAL_TIM_Base_Start_IT(&USMARTTimer_HandleTypeDef); // 使能定时器4和定时器4中断
// }
...
// 初始化串口控制器
// sysclk:系统时钟（Mhz）
// 调用了timer.h；timer14，定时100ms
void usmart_init(u8 sysclk)
{
#if USMART_ENTIMX_SCAN == 1
	USMARTTimer_FunInit((u32)(sysclk * 1000 / 240 - 1), (u32)(sysclk / 2 * 100 - 1)); // 定时器6初始化，周期100ms,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
