...
u8 MPU6050_Init(void)
{
    ...
    printf("MPU6050_ADDR =%x\r\n", res);
    if (res == MPU6050_ADDR) // 器件ID正确,即res = MPU6050_ADDR = 0x68
    {
...
/**********************************************
函数名称：MPU6050_Get_Accelerometer
函数功能：得到加速度值(原始值)
...
u8 MPU6050_Write_Len(u8 addr, u8 reg, u8 len, u8 *buf)
{
    ...
    MPU6050_IIC_Start();
    MPU6050_IIC_Send_Byte((addr << 1) | 0); // 发送器件地址+写命令(0为写,1为读)
...
void Get_Angle(u8 way)
{
    ...
    Temperature = MPU6050_Get_Temperature(); // 读取MPU6050内置温度传感器数据，近似表示主板温度,乘了10。
    if (way == 1)                            // DMP的读取在数据采集中断读取，严格遵循时序要求
    {
        mpu6050_dmp_get_data(&Pitch, &Roll, &Yaw); // 读取加速度、角速度、倾角
    ...
    {
        Gyro_X = (MPU6050_Read_Byte(MPU6050_GYRO_XOUTH_REG) << 8) + MPU6050_Read_Byte(MPU6050_GYRO_XOUTL_REG);    // 读取X轴陀螺仪
        Gyro_Y = (MPU6050_Read_Byte(MPU6050_GYRO_YOUTH_REG) << 8) + MPU6050_Read_Byte(MPU6050_GYRO_YOUTL_REG);    // 读取Y轴陀螺仪
        Gyro_Z = (MPU6050_Read_Byte(MPU6050_GYRO_ZOUTH_REG) << 8) + MPU6050_Read_Byte(MPU6050_GYRO_ZOUTL_REG);    // 读取Z轴陀螺仪
        Accel_X = (MPU6050_Read_Byte(MPU6050_ACCEL_XOUTH_REG) << 8) + MPU6050_Read_Byte(MPU6050_ACCEL_XOUTL_REG); // 读取X轴加速度计
        Accel_Y = (MPU6050_Read_Byte(MPU6050_ACCEL_YOUTH_REG) << 8) + MPU6050_Read_Byte(MPU6050_ACCEL_YOUTL_REG); // 读取X轴加速度计
        Accel_Z = (MPU6050_Read_Byte(MPU6050_ACCEL_ZOUTH_REG) << 8) + MPU6050_Read_Byte(MPU6050_ACCEL_ZOUTL_REG); // 读取Z轴加速度计
        if (Gyro_X > 32768)
