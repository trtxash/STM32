...
#include "stm32f4xx_ll_adc.h"
    ...
    typedef struct
    {
        ...
        FunctionalState ContinuousConvMode;    /*!< Specifies whether the conversion is performed in single mode (one conversion) or continuous mode for regular group,
                                                    ...
                                                    参数可以是使能 ENABLE，表示连续转换或者 DISABLE 禁能，表示单次转换. */
        uint32_t NbrOfConversion;              /*!< Specifies the number of ranks that will be converted within the regular group sequencer.
                                                    To use regular group sequencer and convert several ranks, parameter 'ScanConvMode' must be enabled.
                                                    This parameter must be a number between Min_Data = 1 and Max_Data = 16.
                                                    用于配置规则通道要转换的通道数。
                                                        如果要进行多个通道的转换，参数成员 ScanConvMode 必须使能.
                                                        通道数范围 1 到 16。
                                                        仅当规则通道上没有后续的转换时才可以修改此参数(即 ADC 禁止的情况下或者 ADC 单次转换模式，又或者无外部触发)。 */
        FunctionalState DiscontinuousConvMode; /*!< Specifies whether the conversions sequence of regular group is performed in Complete-sequence/Discontinuous-sequence (main sequence subdivided in successive parts).
                                                    Discontinuous mode is used only if sequencer is enabled (parameter 'ScanConvMode'). If sequencer is disabled, this parameter is discarded.
                                                    Discontinuous mode can be enabled only if continuous mode is disabled. If continuous mode is enabled, this parameter setting is discarded.
                                                    This parameter can be set to ENABLE or DISABLE.
                                                    用于配置ADC规则组转换序列的不连续方式。这里的不连续含义是指每次触发进行一个子组的转换。注意跟参数成员 ContinuousConvMode 的含义区分开。
                                                    ...
                                                    使用此参数要注意以下两点:
														只有参数成员 ScanConvMode 使能的情况下，此参数才有使用的意义，否则忽略此参数。
														只有参数成员 ContinuousConvMode 禁止的情况下,此参数才有使用的意义,否则忽略此参数也就是说这两个参数不能同时使能。*/
